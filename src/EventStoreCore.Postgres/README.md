...
`uuid` primary key `IdempotencyKey`, required `varchar(64)` `RequestHash`,
stream ID/type/tenant columns, previous/current version columns, and a
`timestamp with time zone` timestamp. Add the model-generated lookup index over
`(StreamId, StreamType, TenantId, CurrentVersion)`. Verify that the existing
global unique constraint on `Events.EventId` is present; no event backfill is
required. Deploy schema first, then writers, and retain operation rows for at
least the maximum retry lifetime.
